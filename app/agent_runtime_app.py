...
# 🐒 --- MONKEYPATCH PROTOBUF TO PYDANTIC SERIALIZATION --- 🐒
# The Vertex AI SDK (vertexai._genai._agent_engines_utils) attempts to serialize the A2A AgentCard
# using Protobuf's `json_format.MessageToJson`. However, the ADK defines AgentCard as a Pydantic model.
# This mismatch causes AttributeError: 'AgentCard' object has no attribute 'DESCRIPTOR'.
# We intercept the call and route it to Pydantic's native JSON serializer if a Pydantic model is passed.
from google.protobuf import json_format

original_message_to_json = json_format.MessageToJson

def my_message_to_json(message: Any, *args: Any, **kwargs: Any) -> str:
    if hasattr(message, "model_dump_json"):
        return message.model_dump_json()
    return original_message_to_json(message, *args, **kwargs)

json_format.MessageToJson = my_message_to_json
# 🐒 ----------------------------------------------------- 🐒
